...
/**
 * All registered (tag, slug) pairs for insight detail pages.
 * Used by generateStaticParams to pre-render every insight at build time.
 */
export const INSIGHT_ROUTES: Array<{ tag: string; slug: string }> = [
  INSIGHT_KEY_CONSULTING_AI_BUSINESS_AUTOMATION,
  INSIGHT_KEY_CONSULTING_DIGITAL_TRANSFORMATION,
  INSIGHT_KEY_CONSULTING_DIGITAL_UX,
  INSIGHT_KEY_CONSULTING_BIG_DATA_DRIVEN,
  INSIGHT_KEY_CONSULTING_DIGITAL_TRANSFORMATION_TVP,
  INSIGHT_KEY_CONSULTING_LEGAL_CONSIDERATIONS_MA,
  INSIGHT_KEY_ACCOUNTING_INTRODUCTION_XERO,
  INSIGHT_KEY_ACCOUNTING_ACCOUNTING_KNOWLEDGE,
  INSIGHT_KEY_TAX_DIVIDEND_LEGAL_CONSIDERATIONS,
  INSIGHT_KEY_TAX_VAT,
  INSIGHT_KEY_TAX_CORPORATE_TAX,
  INSIGHT_KEY_CORPORATE_SERVICE_AMENDMENT,
  INSIGHT_KEY_CORPORATE_SERVICE_INCORPORATION,
  INSIGHT_KEY_CORPORATE_SERVICE_CORPORATE_SECRETARY,
  INSIGHT_KEY_HR_TERMINATION_REGULATION,
  INSIGHT_KEY_HR_HYBRID_WORK_PERFORMANCE,
  INSIGHT_KEY_HR_LEAVE_POLICY,
  INSIGHT_KEY_HR_MANDATORY_PROVIDENT_FUND,
  INSIGHT_KEY_HR_IR56,
  INSIGHT_KEY_HR_TAX_CLEARANCE_DEPARTING,
  INSIGHT_KEY_HR_WITHHOLDING_OBLIGATIONS_DEPARTING,
  INSIGHT_KEY_TAX_TWO_TIERED_SALARIES_TAX,
  INSIGHT_KEY_HR_EMPLOYERS_RETURN_BIR56A,
].map((key) => {
  const [tag, slug] = key.split("/");
  return { tag, slug };
});
